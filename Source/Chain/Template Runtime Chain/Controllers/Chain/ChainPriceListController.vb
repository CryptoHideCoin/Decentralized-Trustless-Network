...
Namespace Controllers
    ...
    ' GET: api/{GUID service}/chain/chainPriceListController
    <Route("ChainApi")>
    Public Class ChainPriceListController

        Inherits ApiController


        ''' <summary>
        ''' This method provide to get a price list of a chain
        ''' </summary>
        ''' <returns></returns>
        Public Function GetValue(ByVal name As String) As Models.Chain.Response.ChainPriceListDataModel
            Dim result As New Models.Chain.Response.ChainPriceListDataModel
                ...
                If (AreaCommon.state.serviceInformation.currentStatus = Models.Service.InternalServiceInformation.EnumInternalServiceState.started) Then
                    If (AreaCommon.state.network.position = CHCRuntimeChainLibrary.AreaRuntime.AppState.EnumConnectionState.genesisOperation) Or
                       (AreaCommon.state.network.position = CHCRuntimeChainLibrary.AreaRuntime.AppState.EnumConnectionState.onLine) Then
                        ...
                        With AreaCommon.state.runTimeState.chainByName(name).priceList
                            result.value = .value
