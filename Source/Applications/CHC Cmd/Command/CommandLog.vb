        ...
        Private Function readParameters() As Boolean
            Try
                ...
                If _Command.haveParameter("blockExplorer") Then
                    _ParameterCommand = "blockExplorer"
                End If
                If _Command.haveParameter("deleteBlock") Then
                    _ParameterCommand = "deleteBlock"
                End If
...
#Enable Warning BC42024
        ...
        Private Function getLogList() As Boolean
            Try
                ...
                If proceed Then
                    remote.url = buildURL($"/administration/logList/?securityToken={_SecurityToken}")
                End If
                If proceed Then
                ...
                End If
                If proceed Then
                    Console.WriteLine($"Log list of Service = {_ParameterService}")
                    Console.WriteLine()
...
#Enable Warning BC42016
            ...
            Catch exFile As System.IO.FileLoadException
                ...
                Return False
            Catch ex As Exception
                Console.WriteLine("Error during getLogList - " & ex.Message)
        ...
        Private Function showBlockLog(ByVal value As String) As Boolean
            Try
                ...
                Dim parameterService As String = $"--service:{_ParameterService} "
                Dim parameterDataPath As String = $"--dataPath:{_ParameterDataPath} "
                Dim parameterPassword As String = ""
                Dim parameterLoadBlock As String = $"--loadBlock:{value} "
                Dim parameterAddress As String = $"--address:{_ParameterAddress} "
                ...
                If _ParameterPassword.Length > 0 Then
                    parameterPassword = $"--password:{_ParameterPassword} "
                End If
                If _ParameterSecurityKey.Length > 0 Then
                    parameterSecurityKey = $"--securityKey:{_ParameterSecurityKey} "
                End If
                ...
                If IO.Directory.Exists(path) Then
                    ...
                    Else
                        Console.WriteLine($"{path} -showBlock {parameters}--pause")

                        Process.Start(path, $"-showBlock {parameters} --pause")
                ...
                Return True
            Catch ex As Exception
                Console.WriteLine("Error during showBlockLog - " & ex.Message)
        ...
        Private Function block() As Boolean
            ...
            Catch exFile As System.IO.FileLoadException
                ...
                Return False
            Catch ex As Exception
                Console.WriteLine("Error during block - " & ex.Message)
        ...
        Private Function deleteBlock() As Boolean
            Try
                ...
                If proceed Then
                    remote.url = buildURL($"/administration/logBlockDelete/?securityToken={_SecurityToken}&blockNumber={blockValue}")
                End If
                If proceed Then
                ...
                End If
                If proceed Then
                    Console.WriteLine($"Delete block {blockValue} log Executed")
                End If
            ...
            Catch exFile As System.IO.FileLoadException
                ...
                Return False
            Catch ex As Exception
                Console.WriteLine("Error during deleteBlock - " & ex.Message)
        ...
        Private Function deleteOldInstance() As Boolean
            Try
                ...
                If proceed Then
                    remote.url = buildURL($"/administration/maintenance/cleanOldLogInstance/?securityToken={_SecurityToken}")
                End If
                If proceed Then
                    proceed = (remote.sendData("PUT").Length = 0)
                End If
                ...
                End If
                If proceed Then
                    Console.WriteLine($"Delete old instance log Executed")
                End If
        ...
        Private Function deleteOld() As Boolean
            Try
                ...
                If proceed Then
                    remote.url = buildURL($"/administration/maintenance/cleanLog/?securityToken={_SecurityToken}")
                End If
                If proceed Then
                    proceed = (remote.sendData("PUT").Length = 0)
                End If
                ...
                End If
                If proceed Then
                    Console.WriteLine($"Delete old log Executed")
                End If
                    ...
                    Select Case _ParameterCommand.ToLower()
                        Case "getList".ToLower()
                            If Not getLogList() Then
                                Console.WriteLine("Problem during read the Log Index!")

                                proceed = False
                            End If
                        Case "blockExplorer".ToLower()
                            If Not block() Then
                                Console.WriteLine("Problem during read block!")

                                proceed = False
                            End If
                        Case "deleteBlock".ToLower()
                            If Not deleteBlock() Then
                                Console.WriteLine("Problem during delete block!")

                                proceed = False
                            End If
                        Case "deleteOldLogInstance".ToLower()
                            If Not deleteOldInstance() Then
                                Console.WriteLine("Problem during delete old log instance!")
