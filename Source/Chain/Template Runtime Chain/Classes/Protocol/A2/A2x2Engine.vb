...
Namespace AreaProtocol
    ...
    ''' <summary>
    ''' This class contain all element to manage a A2x2 command
    ''' </summary>
    Public Class A2x2
        ...
        Public Class RequestModel : Implements IRequestModel
            ...
            Public Property content As New EssentialA2x2
        ...
        Public Class RequestResponseModel
            ...
            Public Property common As CommonRequest Implements IRequestModel.common
                ...
                End Set
            End Property
            Public Property content As EssentialA2x2
                Get
                    Return _Base.content
                End Get
                Set(value As EssentialA2x2)
                    _Base.content = value
                End Set
            End Property
        ...
        Public Class FormalCheck
            ...
            Shared Function evaluate(ByRef value As AreaFlow.RequestExtended) As Nullable(Of Boolean)
                Try
                    Dim request As A2x2.RequestModel = value.data
                    Dim chain As AreaState.ChainStateEngine.DataChain
                    ...
                    chain = AreaCommon.state.runTimeState.chainByName(request.content.chainName)

                    If Not chain.serviceNodeList.ContainsKey(request.content.publicAddress) Then
                        value.evaluations.rejectedNote = "Node not active in chain"
                        value.position.verify = AreaFlow.EnumOperationPosition.completeWithNegativeResult
        ...
        Public Class Manager
            ...
            ''' </summary>
            ''' <returns></returns>
            Shared Function addIntoLedger(ByVal approverPublicAddress As String, ByVal consensusHash As String, ByVal registrationTimeStamp As String, ByVal value As EssentialA2x2, ByVal requesterPublicAddress As String, ByVal requestHash As String) As CHCCommonLibrary.AreaCommon.Models.General.IdentifyLastTransaction
                Try
                    ...
                    AreaCommon.log.track("A2x2.Manager.addIntoLedger", "Begin")

                    contentPath = IO.Path.Combine(contentPath, hash & ".Content")

                    If IOFast(Of EssentialA2x2).save(contentPath, value) Then
                ...
                Catch ex As Exception
                    ...
                    AreaCommon.log.track("A2x2.Manager.addIntoLedger", ex.Message, "fatal")

                    Return New CHCCommonLibrary.AreaCommon.Models.General.IdentifyLastTransaction
                Finally
                    AreaCommon.log.track("A2x2.Manager.addIntoLedger", "Completed")
                End Try
            End Function
            ...
            Shared Function createInternalRequest(ByVal publicAddress As String, ByVal chainName As String) As String
                Try
                    ...
                    AreaCommon.log.track("A2x2Manager.createInternalRequest", "Begin")
                    ...
                    If saveTemporallyRequest(data) Then
                        AreaCommon.log.track("A2x2Manager.createInternalRequest", "request - Saved")
                ...
                Catch ex As Exception
                    ...
                    AreaCommon.log.track("A2x2Manager.createInternalRequest", ex.Message, "fatal")
                Finally
                    AreaCommon.log.track("A2x2Manager.createInternalRequest", "Completed")
                End Try
