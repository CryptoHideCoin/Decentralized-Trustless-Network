...
Namespace AreaAsynchronous
    ...
    Module Internal
        ...
        ''' <summary>
        ''' This method provide to execute a counter clean
        ''' </summary>
        ''' <returns></returns>
        Friend Function executeCounterClean() As Boolean
            Dim ownerId As String = "CounterClean-" & Guid.NewGuid.ToString

            Try
                currentNumJobInRun += 1

                AreaCommon.Main.environment.log.trackEnter("AreaAsynchronous.Internal.executeCounterClean", ownerId)

                Dim engine As New AreaEngine.CleanCounterEngine

                engine.ownerId = ownerId

                Return engine.run
            Catch ex As Exception
                AreaCommon.Main.environment.log.trackException("AreaAsynchronous.Internal.executeCounterClean", ownerId, ex.Message)

                Return False
            Finally
                AreaCommon.Main.environment.log.trackExit("AreaAsynchronous.Internal.executeCounterClean", ownerId)
