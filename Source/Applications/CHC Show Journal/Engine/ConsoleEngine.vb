...
Namespace AreaCommon
    ...
    Class ConsoleEngine
        ...
        ''' </summary>
        ''' <returns></returns>
        Private Function readRemotePage() As Boolean
            Try
                Dim remote As New ProxyWS(Of RegistryStreamResponseModel)
                Dim singleLine As String = ""
                Dim numElements As Integer = 0

                remote.url = buildURL($"/administration/registryPage/?securityToken={_SecurityToken}&index={_Date}")

                Console.WriteLine($"Service journal: {_Date}")
                Console.WriteLine("")
                Console.WriteLine("")
